...
class Transformations:
    ...
    def convolution(self, layer, img):
        ...
        plt.imsave(fname, np.array(conv[0, :, :, 0], dtype='float32'), cmap=plt.get_cmap("gray"))
    ...
    def pool(self, img, layer):
        ...
        sig = tf.sigmoid(img)
        # Maxpooling
        max_pool = tf.nn.max_pool(sig, ksize=[1, 2, 2, 1], strides=self.stride, padding=self.padding)
        # Name of the file in which the image is stored
        fname = str("conv_pool" + str(layer) + ".png")
        # Save the pooled, grayscale image
        plt.imsave(fname, np.array(max_pool[0, :, :, 0], dtype='float32'), cmap=plt.get_cmap("gray"))
